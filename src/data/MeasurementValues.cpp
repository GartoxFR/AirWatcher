...
int MeasurementValues::ComputeQualityIndex() const {
    ...
    for(int i=9;i>0;i--){
        if(this->GetO3() > tabO3[i] || this->GetSO2() > tabSO2[i] || this->GetNO2() > tabNO2[i] || this->GetPM10() > tabPM10[i]){
            break;
        }
        indice--;
    }
...
double MeasurementValues::ComputeSimilarity(const MeasurementValues& values) const {
    double value = pow(this->GetO3()-values.GetO3(),2)+pow(this->GetSO2()-values.GetSO2(),2)+pow(this->GetNO2()-values.GetNO2(),2)+pow(this->GetPM10()-values.GetPM10(),2);
    return value;
}
